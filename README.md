...
- A sample belongs to a class when its Hotelling T² and its DModX are both within the class limits at confidence level `alpha` (default 0.95, i.e. 5 % significance). DModX is `sqrt(SSE / (k - A)) / s0`, with `SSE` the residual sum of squares over all of the sample's observed features, `k` the number of its observed features that vary within the class, `A` the number of components and `s0` the pooled residual standard deviation (per degree of freedom) of the class training data; its limit is `NipalsPCA.calc_limit(metric="DModX")`. A deviation in a feature that was constant in the class training data adds to `SSE`, and so rejects the sample only if it pushes DModX over the limit.
